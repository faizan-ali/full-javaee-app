                    ...
                    <div class="container-fluid platform-nav-container">
                        <div class="row platform-nav">
                            ...
                            <div class="hidden-xs">
                                ...
                                <div class="col-md-2 text-center platform-nav-button subhead-1-2 platform-nav-button-border pull-right">Candidate Logs</div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="row row-centered" style="margin-top: 25px; margin-bottom: 65px;">
                            <div class="col-md-10 col-centered">
                                <table class="table table-hover" id="table">
                                    ...
                                    <tbody>
                                        <c:forEach var="elt" items="${totalCandidates}">
                                            <tr class="body-bold-1-1">
                                                <td>
                                                    ${elt.firstName} ${elt.lastName}
                                                </td>
                                                <td>${elt.getTotalLogins()}</td>
                                                <td>${elt.getLastLoginDate()}</td>
                                                <td>${elt.field}
                                                    <p>${elt.pastField}</p>
                                                </td>
                                                <td>${elt.anticipatedCertification}
                                                    <p>${elt.obtainedCertification}</p>
                                                </td>
                                                <td>${elt.school}</td>
                                                <td>${elt.city} ${elt.state}</td>
                                                <td>
                                                    <a href="../AdministratorViewCandidateProfileServlet?candidateID=${elt.candidateID}&candidateLogsList=true">
                                                        <button type="button" class="archer btn btn-sm btn-success">View</button>
                                                    </a>
                                                </td>
                                            </tr>
                                        </c:forEach>
